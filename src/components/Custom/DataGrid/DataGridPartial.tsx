import { createContext, createElement, Fragment, ReactNode, useContext, useState } from "react";
import { FaSort, FaSortDown, FaSortUp, FaTrash } from "react-icons/fa6";
import { Spinner, Table as RSTable, Form, Button, Dropdown, FormControlProps, ButtonProps } from "react-bootstrap";
...
const CreateButton = ({ children = "Ekle", ...props }: ButtonProps) => {
  const { append, columns } = useContext(DataGridContext);

  return (
    <Button
      onClick={() => {
        append(columns.reduce((x: any, y: any) => ({ ...x, [y?.key]: "" }), {}));
        // reset(columns.reduce((x: any, y: any) => ({ ...x, [y?.key]: '' }), { key: body.length }));
      }}
      {...props}
    >
      {children}
...
const Pagination = ({
  ...
  ...props
}: React.ComponentProps<typeof DynaPagination>) => {
  return <DynaPagination classNamePagination={classNamePagination} {...props} />;
};
