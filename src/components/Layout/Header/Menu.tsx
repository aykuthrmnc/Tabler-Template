...
const MenuItem = ({ item }: { item: MenuItemTypes }) => (
  <Nav.Item>
    <Nav.Link onClick={item?.onClick} to={item?.url!} {...(item?.url && { as: NavLink, end: item?.end })} className={item?.className}>
...
const DropdownItem = ({ item }: { item: MenuItemTypes }) => (
  <Dropdown.Item onClick={item?.onClick} to={item?.url!} {...(item?.url && { as: NavLink, end: item?.end })} className={item?.className}>
    {item?.icon && <span className="nav-link-icon d-md-none d-lg-inline-block color-inherit">{item?.icon}</span>}{" "}
    {item?.label && <span className="nav-link-title">{item?.label}</span>}
    {item?.badge && <span className={`badge badge-sm bg-${item?.badge?.variant || "green"}-lt text-uppercase ms-auto`}>{item?.badge?.text}</span>}
  </Dropdown.Item>
);
      ...
      <Dropdown.Menu
        ...
        style={
          item?.children?.length! > 8
            ? { display: "grid", gridTemplateRows: `repeat(${Math.round(item?.children?.length! / 2)}, 1fr)`, gridAutoFlow: "column" }
            : {}
        }
      >
        {item?.children?.map((subItem, key) =>
          subItem?.children ? (
            <MenuItemWithChildren item={subItem} toggleMenu={toggleMenu} activeMenuItems={activeMenuItems} key={key} drop="end" />
          ) : (
